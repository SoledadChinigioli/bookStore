...
export class DataApiService {
  ...
  public selectBook: BookInterface = {id: null};

  getAllBooks(){
    this.bookCollection = this.afs.collection<BookInterface>('books');
    return this.books = this.bookCollection.snapshotChanges().pipe(map(changes =>{
  ...
  getAllBooksOffers() {
    this.bookCollection = this.afs.collection('books', ref => ref.where('oferta', '==', '1'));
